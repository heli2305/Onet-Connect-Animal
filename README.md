...
* `algorithms/`: Các nhóm thuật toán giải bài toán.
  ...
  * `uninformed/`: Thuật toán tìm kiếm mù (BFS, UCS,...).
  * `informed/`: Thuật toán tìm kiếm có thông tin (A*, Greedy,...).
  * `local/`: Thuật toán tìm kiếm cục bộ (Hill-Climbing, Simulated Annealing,...).
  * `complex/`: Thuật toán tìm kiếm trong môi trường phức tạp (AND-OR Search, Partial Observable,...).
  * `csp/`: Bài toán thỏa mãn ràng buộc (Backtracking, AC-3,...).
  * `adversarial/`: Thuật toán tìm kiếm đối kháng (Minimax, Alpha-Beta,...).
